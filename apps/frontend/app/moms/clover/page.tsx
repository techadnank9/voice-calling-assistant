...
function StatusBadge({ status }: { status: string | null }) {
  if (!status) {
    return <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-semibold text-slate-500">Pending</span>;
  }
  if (status === 'sent') {
    return <span className="rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs font-semibold text-emerald-800">✓ Sent</span>;
  }
  if (status === 'failed') {
    return <span className="rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-semibold text-red-800">✗ Failed</span>;
  }
  ...
  return (
    <OpsShell active="clover">
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Items Total</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Clover Order ID</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map(o => (
                  <tr key={o.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-4 py-3">
                      {o.clover_order_id ? (
                        <span className="font-mono text-xs text-slate-500">{o.clover_order_id}</span>
                      ) : '—'}
                    </td>
      ...
      <section className="mt-6 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 space-y-1">
        <p><strong>Sent</strong> — order pushed to Clover successfully. Clover Order ID is shown.</p>
        <p><strong>Failed</strong> — push failed. Error message shown. Check Railway logs for details.</p>
        <p><strong>Pending</strong> — order exists in Supabase but Clover push hasn't run yet (or Clover not configured).</p>
      </section>
    </OpsShell>
  );
}
